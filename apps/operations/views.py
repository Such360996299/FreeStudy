...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        ...
        courses = Course.objects.filter(is_banner=False)[:6] #最新上线的课程
        banner_courses = Course.objects.filter(is_banner=True)
        course_orgs = CourseOrg.objects.all().order_by("-click_nums")[:15] #热门机构按点选数排序
        teacher = Teacher.objects.all().order_by("-click_nums")[:15] #首页展示的讲师按点击数排序
